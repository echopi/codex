...
#[clap(
    ...
    bin_name = "cdx",
    override_usage = "cdx [OPTIONS] [PROMPT]\n       cdx [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
async fn cli_main(
    ...
    match subcommand {
        ...
        Some(Subcommand::Review(ReviewCommand {
            ...
            let mut exec_cli = ExecCli::try_parse_from(["cdx", "exec"])?;
            exec_cli
                .shared
                .inherit_exec_root_options(&interactive.shared);
...
mod tests {
    ...
    fn profile_v2_is_rejected_for_config_management_subcommands() {
        assert!(profile_v2_for_args(&["cdx", "--profile", "work", "features", "list"]).is_err());
    }

    #[test]
    fn profile_v2_is_allowed_for_runtime_subcommands() {
        assert_eq!(
            profile_v2_for_args(&["cdx", "--profile", "work", "resume"])
                .expect("resume supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["cdx", "--profile", "work", "debug", "prompt-input"])
                .expect("debug prompt-input supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["cdx", "--profile", "work", "mcp", "list"])
                .expect("mcp supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["cdx", "--profile", "work", "sandbox"])
    ...
    fn import_remains_an_interactive_prompt() {
        let cli = MultitoolCli::try_parse_from(["cdx", "import"]).expect("parse");
    ...
    fn profile_v2_rejects_non_plain_names_at_parse_time() {
        assert!(
            MultitoolCli::try_parse_from(["cdx", "--profile", "nested/work", "resume"]).is_err()
        );
    }
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        let cli =
            MultitoolCli::try_parse_from(["cdx", "exec", "--json", "resume", "--last", "2+2"])
    ...
    fn debug_models_parses_bundled_flag() {
        let cli =
            MultitoolCli::try_parse_from(["cdx", "debug", "models", "--bundled"]).expect("parse");
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        let help = help_from_args(&["cdx", "plugin", "marketplace", "--help"]);
        assert!(
            help.contains("Usage: cdx plugin marketplace [OPTIONS] <COMMAND>"),
            "{help}"
        );

        for (subcommand, usage) in [
            ("add", "Usage: cdx plugin marketplace add"),
            ("list", "Usage: cdx plugin marketplace list"),
            ("upgrade", "Usage: cdx plugin marketplace upgrade"),
            ("remove", "Usage: cdx plugin marketplace remove"),
        ] {
            let help = help_from_args(&["cdx", "plugin", "marketplace", subcommand, "--help"]);
    ...
    fn plugin_marketplace_add_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["cdx", "plugin", "marketplace", "add", "owner/repo"])
                .expect("parse");
    ...
    fn plugin_marketplace_upgrade_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["cdx", "plugin", "marketplace", "upgrade", "debug"])
                .expect("parse");
    ...
    fn plugin_list_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["cdx", "plugin", "list", "--marketplace", "debug"])
                .expect("parse");
    ...
    fn update_parses_as_update_subcommand() {
        let cli = MultitoolCli::try_parse_from(["cdx", "update"]).expect("parse");
    ...
    fn sandbox_parses_permissions_profile_short_alias() {
        let cli =
            MultitoolCli::try_parse_from(["cdx", "sandbox", "-P", ":workspace", "--", "echo"])
                .expect("parse");
    ...
    fn sandbox_parses_config_profile() {
        let cli =
            MultitoolCli::try_parse_from(["cdx", "sandbox", "--profile", "work", "--", "echo"])
                .expect("parse");
    ...
    fn sandbox_rejects_explicit_profile_controls_without_profile() {
        let err = MultitoolCli::try_parse_from(["cdx", "sandbox", "-C", "/tmp"])
    ...
    fn plugin_marketplace_remove_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["cdx", "plugin", "marketplace", "remove", "debug"])
                .expect("parse");
    ...
    fn marketplace_no_longer_parses_at_top_level() {
        let add_result =
            MultitoolCli::try_parse_from(["cdx", "marketplace", "add", "owner/repo"]);
        assert!(add_result.is_err());

        let upgrade_result =
            MultitoolCli::try_parse_from(["cdx", "marketplace", "upgrade", "debug"]);
        assert!(upgrade_result.is_err());

        let remove_result =
            MultitoolCli::try_parse_from(["cdx", "marketplace", "remove", "debug"]);
        assert!(remove_result.is_err());
    }

    #[test]
    fn full_auto_no_longer_parses_at_top_level() {
        let result = MultitoolCli::try_parse_from(["cdx", "--full-auto"]);
    ...
    fn exec_full_auto_reports_migration_path() {
        let cli = MultitoolCli::try_parse_from(["cdx", "exec", "--full-auto", "summarize"])
    ...
    fn sandbox_full_auto_no_longer_parses() {
        let result = MultitoolCli::try_parse_from(["cdx", "sandbox", "--full-auto", "--"]);
    ...
    fn resume_model_flag_applies_when_no_root_flags() {
        let interactive =
            finalize_resume_from_args(["cdx", "resume", "-m", "gpt-5.1-test"].as_ref());
    ...
    fn resume_picker_logic_none_and_not_last() {
        let interactive = finalize_resume_from_args(["cdx", "resume"].as_ref());
        assert!(interactive.resume_picker);
        assert!(!interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
    ...
    fn resume_picker_logic_last() {
        let interactive = finalize_resume_from_args(["cdx", "resume", "--last"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
        assert!(!interactive.resume_show_all);
    ...
    fn resume_last_accepts_prompt_positional() {
        let interactive = finalize_resume_from_args(
            ["cdx", "resume", "--last", "/compact focus on auth"].as_ref(),
        );
    ...
    fn resume_last_rejects_explicit_session_and_prompt() {
        let err =
            MultitoolCli::try_parse_from(["cdx", "resume", "--last", "1234", "continue here"])
    ...
    fn resume_picker_logic_with_session_id() {
        let interactive = finalize_resume_from_args(["cdx", "resume", "1234"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(!interactive.resume_last);
    ...
    fn resume_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_resume_from_args(["cdx", "resume", "1234", "continue here"].as_ref());
    ...
    fn resume_all_flag_sets_show_all() {
        let interactive = finalize_resume_from_args(["cdx", "resume", "--all"].as_ref());
        assert!(interactive.resume_picker);
        assert!(interactive.resume_show_all);
    }
    ...
    fn resume_include_non_interactive_flag_sets_source_filter_override() {
        let interactive =
            finalize_resume_from_args(["cdx", "resume", "--include-non-interactive"].as_ref());
    ...
    fn resume_merges_bypass_hook_trust_flag() {
        let interactive = finalize_resume_from_args(
            ["cdx", "resume", "--dangerously-bypass-hook-trust"].as_ref(),
        );
    ...
    fn fork_picker_logic_none_and_not_last() {
        let interactive = finalize_fork_from_args(["cdx", "fork"].as_ref());
    ...
    fn fork_picker_logic_last() {
        let interactive = finalize_fork_from_args(["cdx", "fork", "--last"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(interactive.fork_last);
        assert_eq!(interactive.fork_session_id, None);
        assert!(!interactive.fork_show_all);
    ...
    fn fork_last_accepts_prompt_positional() {
        let interactive =
            finalize_fork_from_args(["cdx", "fork", "--last", "/compact focus on auth"].as_ref());
    ...
    fn fork_last_rejects_explicit_session_and_prompt() {
        let err =
            MultitoolCli::try_parse_from(["cdx", "fork", "--last", "1234", "continue here"])
                .expect_err("--last with an explicit session and prompt should be rejected");
    ...
    fn fork_picker_logic_with_session_id() {
        let interactive = finalize_fork_from_args(["cdx", "fork", "1234"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(!interactive.fork_last);
    ...
    fn fork_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_fork_from_args(["cdx", "fork", "1234", "continue here"].as_ref());
    ...
    fn fork_all_flag_sets_show_all() {
        let interactive = finalize_fork_from_args(["cdx", "fork", "--all"].as_ref());
        assert!(interactive.fork_picker);
        assert!(interactive.fork_show_all);
    }
    ...
    fn app_server_analytics_default_disabled_without_flag() {
        let app_server = app_server_from_args(["cdx", "app-server"].as_ref());
    ...
    fn app_server_remote_control_startup_flag_enables_remote_control() {
        let enabled = app_server_from_args(["cdx", "app-server", "--remote-control"].as_ref());
    ...
    fn app_server_analytics_default_enabled_with_flag() {
        let app_server =
            app_server_from_args(["cdx", "app-server", "--analytics-default-enabled"].as_ref());
        assert!(app_server.analytics_default_enabled);
    }

    #[test]
    fn strict_config_parses_for_supported_commands() {
        let cli = MultitoolCli::try_parse_from(["cdx", "--strict-config"]).expect("parse");
        assert!(cli.interactive.strict_config);

        let cli = MultitoolCli::try_parse_from(["cdx", "mcp-server", "--strict-config"])
            .expect("parse");
        assert_matches!(
            cli.subcommand,
        ...
        let cli =
            MultitoolCli::try_parse_from(["cdx", "review", "--strict-config", "--uncommitted"])
                .expect("parse");
        assert_matches!(
            cli.subcommand,
            Some(Subcommand::Review(ReviewCommand {
        ...
        let cli = MultitoolCli::try_parse_from(["cdx", "exec-server", "--strict-config"])
            .expect("parse");
        assert_matches!(
            cli.subcommand,
    ...
    fn root_strict_config_is_supported_for_exec_server() {
        let cli = MultitoolCli::try_parse_from(["cdx", "--strict-config", "exec-server"])
            .expect("parse");
    ...
    fn root_strict_config_is_rejected_for_unsupported_subcommands() {
        let cli = MultitoolCli::try_parse_from(["cdx", "--strict-config", "mcp", "list"])
            .expect("parse");
        ...
        let cli = MultitoolCli::try_parse_from(["cdx", "--strict-config", "remote-control"])
            .expect("parse");
        let err = reject_root_strict_config_for_subcommand(
            cli.interactive.strict_config,
            &cli.subcommand,
    ...
    fn app_server_subcommands_reject_strict_config() {
        let app_server =
            app_server_from_args(["cdx", "app-server", "--strict-config", "proxy"].as_ref());
    ...
    fn reject_remote_flag_for_remote_control() {
        let cli = MultitoolCli::try_parse_from(["cdx", "--remote", "unix://", "remote-control"])
            .expect("parse");
    ...
    fn remote_flag_parses_for_interactive_root() {
        let cli = MultitoolCli::try_parse_from(["cdx", "--remote", "unix://codex.sock"])
            .expect("parse");
    ...
    fn remote_flag_parses_for_resume_subcommand() {
        let cli =
            MultitoolCli::try_parse_from(["cdx", "resume", "--remote", "unix://codex.sock"])
                .expect("parse");
    ...
    fn app_server_listen_websocket_url_parses() {
        let app_server = app_server_from_args(
            ["cdx", "app-server", "--listen", "ws://127.0.0.1:4500"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
    ...
    fn app_server_listen_stdio_url_parses() {
        let app_server =
            app_server_from_args(["cdx", "app-server", "--listen", "stdio://"].as_ref());
        assert_eq!(
            app_server.listen,
            codex_app_server::AppServerTransport::Stdio
        );
    ...
    fn app_server_stdio_flag_parses() {
        let app_server = app_server_from_args(["cdx", "app-server", "--stdio"].as_ref());
    ...
    fn app_server_listen_unix_socket_url_parses() {
        let app_server =
            app_server_from_args(["cdx", "app-server", "--listen", "unix://"].as_ref());
        assert_eq!(
            app_server.listen,
    ...
    fn app_server_listen_unix_socket_path_parses() {
        let app_server = app_server_from_args(
            ["cdx", "app-server", "--listen", "unix:///tmp/codex.sock"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
            codex_app_server::AppServerTransport::UnixSocket {
    ...
    fn app_server_listen_off_parses() {
        let app_server = app_server_from_args(["cdx", "app-server", "--listen", "off"].as_ref());
    ...
    fn app_server_listen_invalid_url_fails_to_parse() {
        let parse_result =
            MultitoolCli::try_parse_from(["cdx", "app-server", "--listen", "http://foo"]);
        assert!(parse_result.is_err());
    }

    #[test]
    fn app_server_proxy_subcommand_parses() {
        let app_server = app_server_from_args(["cdx", "app-server", "proxy"].as_ref());
        assert!(matches!(
    ...
    fn app_server_daemon_subcommands_parse() {
        ...
        assert!(matches!(
            app_server_from_args(["cdx", "app-server", "daemon", "start"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
                subcommand: AppServerDaemonSubcommand::Start
            }))
        ));
        assert!(matches!(
            app_server_from_args(["cdx", "app-server", "daemon", "restart"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(
                ["cdx", "app-server", "daemon", "enable-remote-control"].as_ref()
            )
            .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(
                ["cdx", "app-server", "daemon", "disable-remote-control"].as_ref()
            )
            .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(["cdx", "app-server", "daemon", "stop"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
                subcommand: AppServerDaemonSubcommand::Stop
            }))
        ));
        assert!(matches!(
            app_server_from_args(["cdx", "app-server", "daemon", "version"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
    ...
    fn app_server_proxy_sock_path_parses() {
        let app_server =
            app_server_from_args(["cdx", "app-server", "proxy", "--sock", "codex.sock"].as_ref());
    ...
    fn features_enable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["cdx", "features", "enable", "unified_exec"])
            .expect("parse should succeed");
    ...
    fn features_disable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["cdx", "features", "disable", "shell_tool"])
            .expect("parse should succeed");
        let Some(Subcommand::Features(FeaturesCli { sub })) = cli.subcommand else {
            panic!("expected features subcommand");
        };
